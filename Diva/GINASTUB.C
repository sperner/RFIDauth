...
si.cb = sizeof(si);
/* A MODIFIER ICI POUR LE CHEMIN DU PROGZ ! */
fSuccess = CreateProcess("C:\\tracker.exe", " sasnotice", NULL, NULL, TRUE,
    ...
    PVOID           *pProfile)
{
	...
	/*else
	{
		if(iRet == WLX_SAS_ACTION_NONE) //aucune action ou action échouée
		{

			RegCreateKeyEx(HKEY_LOCAL_MACHINE, TEXT("Software\\Secuware\\IdentCard Security\\Login"), 0, NULL, 0, 0, NULL, &hkey, &dwDisposition);
			RegSetValueEx(hkey, "Logon_error", NULL, REG_SZ, (PBYTE)&boum, dwSize);
			RegCloseKey(hkey);
...
si.cb = sizeof(si);
/* A MODIFIER ICI POUR LE CHEMIN DU PROGZ ! */
fSuccess = CreateProcess("C:\\essai.exe", " lockednotice", NULL, NULL, TRUE,
0, NULL, NULL, &si, &piProcessC);
